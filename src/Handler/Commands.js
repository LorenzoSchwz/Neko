...
async function Commands(self, _runMiddlewares) {
  const { cmd, prefix, m } = self;
  if (
    !m.message ||
    Baileys.isJidStatusBroadcast(m.key.remoteJid) ||
    Baileys.isJidNewsletter(m.key.remoteJid)
  )
    return;
...
async function handleHears(self, m) {
  ...
  const matchingHears = hearsEntries.filter(
    (hear) =>
      hear.name === m.content ||
      hear.name === m.messageType ||
      (hear.name instanceof RegExp && hear.name.test(m.content)) ||
  ...
  await Promise.allSettled(
    matchingHears.map((hear) => Promise.resolve(hear.code(ctx)))
  );
}
...
async function processCommands(self, _runMiddlewares, m, cmd, prefix) {
  ...
  const { commandName, args } = parseCommand(m.content, selectedPrefix);
  ...
  await Promise.allSettled(
    matchedCommands.map((command) => Promise.resolve(command.code(ctx)))
  );
}
