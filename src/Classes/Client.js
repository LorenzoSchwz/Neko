...
class Client {
  constructor(opts) {
    ...
    if (Array.isArray(this.prefix) && this.prefix.includes(''))
      this.prefix.sort((a, b) => (a === '' ? 1 : b === '' ? -1 : 0));
  ...
  async _resolveCitation() {
    ...
    for (const [citationName, citationList] of Object.entries(
      this.rawCitation
    )) {
      ...
      for (const citationItem of citationList) {
        ...
        const lidResult = await this.core.getLidUser(
          cleanCitation + Baileys.S_WHATSAPP_NET
        );
        if (lidResult?.[0]) {
  ...
  _onEvents() {
    ...
    try {
      Object.assign(
        this.pushNames,
        JSON.parse(fs.readFileSync(this.pushnamesPath).toString())
      );
    } catch (error) {
      fs.writeFileSync(this.pushnamesPath, JSON.stringify(this.pushNames));
    }
  ...
  async launch() {
    ...
    if (this.usePairingCode && !this.core.authState.creds.registered) {
      ...
      if (this.printQRInTerminal) {
        this.consolefy.error(
          'If you are set usePairingCode to true then you need to set printQRInTerminal to false.'
        );
      ...
      if (!this.phoneNumber) {
        this.consolefy.error(
          'phoneNumber options are required if you are using usePairingCode.'
        );
        this.consolefy.resetTag();
        return;
      }
      ...
      const PHONENUMBER_MCC = (
        await fetch(
          'https://gist.githubusercontent.com/itsreimau/3da60a4937a66e4b1ac34970500e926b/raw/5f4a57faf6d0133c37db60192915d4686e865329/PHONENUMBER_MCC.json'
        )
      ).json();
      if (!PHONENUMBER_MCC.some((mcc) => this.phoneNumber.startsWith(mcc))) {
        this.consolefy.error(
          'phoneNumber format must be like: 62xxx (starts with country code).'
        );
        this.consolefy.resetTag();
        return;
      }
      ...
      setTimeout(async () => {
        const code = this.customPairingCode
          ? await this.core.requestPairingCode(
              this.phoneNumber,
              this.customPairingCode
            )
