...
class GroupData {
  ...
  async isMemberExist(jid) {
    ...
    const check = members.some(
      (member) =>
        Baileys.jidNormalizedUser(member.id) ===
          Baileys.jidNormalizedUser(jid) ||
        Baileys.jidNormalizedUser(member.lid) === Baileys.jidNormalizedUser(jid)
    );
    return check;
  }
  ...
  async isAdmin(jid) {
    ...
    const check = members.filter(
      (member) =>
        (Baileys.jidNormalizedUser(member.id) ===
          Baileys.jidNormalizedUser(jid) ||
          Baileys.jidNormalizedUser(member.lid) ===
            Baileys.jidNormalizedUser(jid)) &&
  ...
  async pendingMembersUpdate(members, action) {
    return await this.ctx._client.groupRequestParticipantsUpdate(
      this.jid,
      members,
      action
    );
